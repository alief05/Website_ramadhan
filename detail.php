...
<section class="ket_pembelian">
      ...
      <h1 style="text-align: center; margin-top: 50px;">Daftar Pembelian</h1><hr style=" width:30%;"> 
        ...
        <?php $nomor = 1; ?>
          ...
          <?php $ambil_prd = $conn->query("SELECT * FROM pembelian_newproduk JOIN produk_baru ON pembelian_newproduk.id_newproduk=produk_baru.id_newproduk WHERE pembelian_newproduk.id_pembelian='$_GET[id]' "); ?>

    <?php  while($detailis = $ambil_prd -> fetch_assoc()){?>
          <tr>
            <td><?= $nomor; ?></td>
            <td><?php  echo $detailis['nama_newproduk']; ?></td>
            <td><?php echo $detailis['harga_newproduk'];?></td>
            <td><?php echo $detailis['jumlah']?></td>
            <td><?php echo $detailis['harga_newproduk']*$detailis['jumlah']; ?></td>
          </tr>
